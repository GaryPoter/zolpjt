...
class ExampleSpider(Spider):
    ...
    def parse_detail_page(self, response):
        ...
        # 获取参数详情页的链接
        url = response.urljoin(response.xpath("//*[@id='_j_tag_nav']/ul/li[2]/a[text()='参数']").xpath("@href")[0].extract())
    ...
    def parse_param_page(self, response):
        try:
            cpu_name = (response.xpath("//*/div[@class='detailed-parameters']/table[1]//tr/th/a[text()='CPU型号']/../../td/span/text()").extract() or
                        response.xpath("//*/div[@class='detailed-parameters']/table[1]//tr/th/a[text()='CPU型号']/../../td/span/text()").extract())[0]
            os_v = (response.xpath("//*/div[@class='detailed-parameters']/table[1]//tr/th/span[text()='出厂系统内核']/../../td/span/text()").extract() or
                    response.xpath("//*/div[@class='detailed-parameters']/table[1]//tr/th/span[text()='出厂系统内核']/../../td/span/a/text()").extract())[0]
            fenbianlv = (response.xpath("//*/div[@class='detailed-parameters']/table[2]//tr/th/a[text()='主屏分辨率']/../../td/span/text()").extract() or
                         response.xpath("//*/div[@class='detailed-parameters']/table[2]//tr/th/a[text()='主屏分辨率']/../../td/span/a/text()").extract())[0]
            if fenbianlv[-1] == '>':
                fenbianlv = fenbianlv[:-1]
            if cpu_name is None:
                print("None : %s" % response._url)
            params = {'resolution': fenbianlv,
                      'cpu': cpu_name,
                      'os': os_v}
            for param in params:
                # legal_param_name_field = param_name = param.xpath("text()")[0].extract()
                legal_param_name_field = param_name = param
            ...
            for row in results:
                print(row, response._url, end='\n')  # 其实这里有个小小的编码问题需要解决
            # 最后愉快的用scrapy crawl zol 启动爬虫吧！
        except IndexError:
            print("out of range: %s" % response._url)
